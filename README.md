...
- **Create Diagram (v5)** — real icon-based top toolbar (zoom in/out, fit-to-view, undo/redo, delete, download PNG) and an icon-tile shape palette beside the canvas with 13 shapes (box, process, circle, decision, triangle, database/cylinder, hexagon, input/output parallelogram, cloud, callout, actor/person, document, text), drag nodes to position, drag from edge handles to connect nodes, fill color picker (7 swatches, floating toolbar on a selected node), **waypoints with visible controls** — select a connector to reveal a green "+" button on the line (click to add a bend point there), click an existing waypoint to reveal a red "×" button beside it (click to remove), drag any waypoint to reshape the line, click to select + delete, double-click/double-tap to rename, undo/redo history, scroll to zoom + drag to pan, connector line-style picker (straight / curved / right-angle), save/load, Download as PNG (waypoints and fill colors included in the exported image)
