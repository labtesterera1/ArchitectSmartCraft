...
function showNodeFloat(node) {
  ...
  // Border style buttons — inline SVG lines showing solid / dashed / dotted
  const borderRow = BORDER_STYLES.map(b => {
    ...
    return `<button data-nborder="${b.key}" title="${b.label}"
      style="width:30px;height:20px;padding:0;cursor:pointer;border-radius:4px;border:1px solid ${active?"var(--color-primary)":"transparent"};
      background:${active?"var(--color-primary-dim,rgba(212,255,58,.15))":"transparent"};
  ...
  // Stroke color dots (border color)
  const strokeRow = STROKE_COLORS.map(c => {
    const active = (node.strokeColor||"") === c.key;
    const bg = c.hex || "transparent";
    return `<button data-nstroke="${c.key}" title="Border: ${c.label}"
      style="width:16px;height:16px;border-radius:50%;background:${bg};
      border:2px solid ${active?"#fff":"rgba(255,255,255,.25)"};padding:0;cursor:pointer;flex-shrink:0;
      ${!c.hex?"box-shadow:inset 0 0 0 1px rgba(255,255,255,.3);position:relative;overflow:hidden;":""}">
  ...
  el.float.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:5px;padding:3px 2px">
      <div style="display:flex;align-items:center;gap:3px">
        <span style="font-size:9px;color:var(--color-text-tertiary);min-width:30px">Fill</span>
        ${sep}${fillRow}${sep}
      ...
      <div style="display:flex;align-items:center;gap:3px">
        <span style="font-size:9px;color:var(--color-text-tertiary);min-width:30px">Border</span>
